...
const Deluxe = () => {
  // Комната "Делюкс" имеет ID 6
  const current_room = rooms.find(room => room.id === 6);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 6);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/delux'].title}
        description={seo_config['prices/delux'].description}
        keywords={seo_config['prices/delux'].keywords}
        canonical_url={seo_config['prices/delux'].canonical_url}
      />
