...
const ComfortPlusSTerrasoi = () => {
  // Комната "Комфорт+ с террасой" имеет ID 4
  const current_room = rooms.find(room => room.id === 4);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 4);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/comfort-plus-s-terrasoi'].title}
        description={seo_config['prices/comfort-plus-s-terrasoi'].description}
        keywords={seo_config['prices/comfort-plus-s-terrasoi'].keywords}
        canonical_url={seo_config['prices/comfort-plus-s-terrasoi'].canonical_url}
      />
...
export default ComfortPlusSTerrasoi; 