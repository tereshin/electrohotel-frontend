...
const ComfortPlus = () => {
  // Комната "Комфорт+" имеет ID 3
  const current_room = rooms.find(room => room.id === 3);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 3);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/comfort-plus'].title}
        description={seo_config['prices/comfort-plus'].description}
        keywords={seo_config['prices/comfort-plus'].keywords}
        canonical_url={seo_config['prices/comfort-plus'].canonical_url}
      />
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h3 className="font-normal text-hotel-darkest-green mb-3 text-lg font-commissioner">Размещение</h3>
