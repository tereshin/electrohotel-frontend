...
const DeluxeSplit = () => {
  // Комната "Делюкс Твин" имеет ID 7
  const current_room = rooms.find(room => room.id === 7);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 7);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/delux-split'].title}
        description={seo_config['prices/delux-split'].description}
        keywords={seo_config['prices/delux-split'].keywords}
        canonical_url={seo_config['prices/delux-split'].canonical_url}
      />
