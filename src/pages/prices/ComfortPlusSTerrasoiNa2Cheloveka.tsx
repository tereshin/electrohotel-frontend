...
const ComfortPlusSTerrasoiNa2Cheloveka = () => {
  // Комната "Комфорт+ с террасой на 2 человека" имеет ID 5
  const current_room = rooms.find(room => room.id === 5);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 5);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/comfort-plus-s-terrasoi-na-2-cheloveka'].title}
        description={seo_config['prices/comfort-plus-s-terrasoi-na-2-cheloveka'].description}
        keywords={seo_config['prices/comfort-plus-s-terrasoi-na-2-cheloveka'].keywords}
        canonical_url={seo_config['prices/comfort-plus-s-terrasoi-na-2-cheloveka'].canonical_url}
      />
...
export default ComfortPlusSTerrasoiNa2Cheloveka; 