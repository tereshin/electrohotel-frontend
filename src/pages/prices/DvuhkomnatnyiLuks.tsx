...
const DvuhkomnatnyiLuks = () => {
  // Комната "Двухкомнатный Люкс" имеет ID 8
  const current_room = rooms.find(room => room.id === 8);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 8);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/dvuhkomnatnyi-luks'].title}
        description={seo_config['prices/dvuhkomnatnyi-luks'].description}
        keywords={seo_config['prices/dvuhkomnatnyi-luks'].keywords}
        canonical_url={seo_config['prices/dvuhkomnatnyi-luks'].canonical_url}
      />
...
export default DvuhkomnatnyiLuks; 