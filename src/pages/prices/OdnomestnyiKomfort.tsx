...
const OdnomestnyiKomfort = () => {
  // Комната "Одноместный Комфорт" имеет ID 1
  const current_room = rooms.find(room => room.id === 1);
  // Остальные комнаты для слайдера (исключаем текущую)
  const other_rooms = rooms.filter(room => room.id !== 1);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden max-w-[2048px] mx-auto">
      <SEO 
        title={seo_config['prices/odnomestnyi-komfort'].title}
        description={seo_config['prices/odnomestnyi-komfort'].description}
        keywords={seo_config['prices/odnomestnyi-komfort'].keywords}
        canonical_url={seo_config['prices/odnomestnyi-komfort'].canonical_url}
      />
...
export default OdnomestnyiKomfort; 